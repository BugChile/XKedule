...
class App extends Component {
  ...
  linkEvent() {
    this.state.linkComponent
      ? this.setState({ linkComponent: null })
      : this.setState({ linkComponent: true });
  }
  ...
  onClickReturn() {
    ...
    this.setState({
      aux_view_time: this.state.current_time,
      refresh_aux: aux_bool
    });
  }
  ...
  generateComponents(
    ...
    content_container_components.push(
      this.switchCard(this.state.mode, events, hashed_by_date)
    );
  ...
  saveNewEvent() {
    ...
    const date_start = toDataDate(
      new_event_object.date,
      new_event_object.date_start
    );
    const date_end = toDataDate(
      new_event_object.date,
      new_event_object.date_end
    );
  ...
  updateEvent() {
    ...
    // this is necessary, for some reason without this date_start gets set to date_end
    const date_start = toDataDate(
      this.new_event_object.date,
      this.new_event_object.date_start
    );
    const date_end = toDataDate(
      this.new_event_object.date,
      this.new_event_object.date_end
    );
    ...
    const additions = this.new_event_object.tag_ids.filter(
      tag_id =>
        this.state.events[this.state.editing_event_id].tag_ids.indexOf(
          tag_id
        ) === -1
    );
  ...
  componentDidMount() {
    ...
    this.intervalIDAUX = setInterval(() => this.tick_current_time(this), 1000);
    this.setMainButtonFunction(this.expand);
