...
import { dateToWrittenDate } from "../../js_helpers/parsers";
import { checkTodayFunction, eventsFromHashed } from "../../js_helpers/helpers";
import { daysLowerCase } from "../../js_helpers/constants";
...
export default class MonthlyCard extends React.Component {
  ...
  generateTaskCards(day, events, hashed_events, tasks_per_cell, day_id) {
    ...
    if (day_events) {
      day_events.forEach((event, index) => {
        task_cards.push(
          <MonthlyTaskCard
            key={`MonthlyTaskCard${index}`}
            event={event}
            clickEvent={this.props.clickEvent}
          />
        );
      });
      if (tasks_per_cell && task_cards.length > tasks_per_cell) {
        // add "see more"
        ...
        task_cards.push(
          <MonthlyTaskCard
            key={`MonthlyTaskCard2${day_id}`}
            event={{
              id: "see_more_" + day_string,
              day: new Date(day),
              title: `See ${not_shown_count} more`
            }}
            clickEvent={(card, id) => {
              this.seeMore(card, events, hashed_events, day_id);
            }}
  ...
  calculateMaxTasks() {
    const next_month =
      this.props.aux_view_time.getMonth() === 11
        ? 0
        : this.props.aux_view_time.getMonth() + 1;
    const week_count = get_day_occurrence(
      new Date(this.props.aux_view_time.getFullYear(), next_month, 0)
    ); //number of weeks on this month
    const cell_height =
      document.getElementById("content").clientHeight / week_count - 40;
  ...
  generateDayCells(
    ...
    var day_date = new Date(
      aux_view_time.getFullYear(),
      aux_view_time.getMonth(),
      1
    ); //first day of month
    ...
    while (
      day_date.getDay() !== 1 ||
      day_date.getMonth() !== (aux_view_time.getMonth() + 1) % 12
    ) {
      //define cell style
      ...
      if (i < 7) {
        day_info.push(
          <div key={`day_week${i}`} className="day_cell_name">
            {daysLowerCase[i]}
          </div>
        );
      }
      day_info.push(
        <DayMonth
          key={`day_month${day_date.getDay()}${i}`}
          onClickDay={this.props.onClickDay}
          classCss={cell_number_class_list}
          day={new Date(day_date)}
        />
      );

      day_id = "monthly_cell" + day_date.getTime();
      day_tasks = this.generateTaskCards(
        day_date,
        events,
        hashed_events,
        tasks_per_cell,
      ...
      day_cells[i] = (
        <MonthlyCardCell
          key={`monthly_card_cell${day_date.getDay()}${i}`}
          day_tasks={day_tasks}
          cell_key={day_id}
          day_info={day_info}
  ...
  seeMore(see_more_card, events, hashed_events, day_id) {
    const expanded_tasks = this.generateTaskCards(
      see_more_card.day,
      events,
      hashed_events
    );
    const expanded_day = dateToWrittenDate(see_more_card.day);
    const expanded_class = "expanded_month_cell visible";
    const expanded = document.getElementById("monthly_cell_expanded");
    const day_cell_props = document
      .getElementById(day_id)
      .getBoundingClientRect();
    const content_props = document
      .getElementById("content")
      .getBoundingClientRect();

    expanded.style.top =
      Math.min(
        day_cell_props.top - 10,
        content_props.bottom + 30 - 22 * expanded_tasks.length
      ) + "px";
    expanded.style.left =
      Math.max(
        day_cell_props.left - day_cell_props.width - 12,
        day_cell_props.width
      ) + "px";
  ...
  componentDidMount() {
    ...
    this.setState({ tasks_per_cell: this.calculateMaxTasks() });
  }
    ...
    return (
      <div className="content_card" id="content_card">
        <div className="content_header">
          {(() => {
            var day_date = new Date(
              this.props.aux_view_time.getFullYear(),
              this.props.aux_view_time.getMonth(),
              1
            );
            while (
              day_date.getDay() !== 1 ||
              day_date.getMonth() !==
                (this.props.aux_view_time.getMonth() + 1) % 12
            ) {
        ...
        <div className="content" id="content">
          {
            <MonthlyCardCell
              day_tasks={this.state.expanded_tasks}
