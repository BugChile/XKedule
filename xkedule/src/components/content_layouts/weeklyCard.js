...
export default class WeeklyCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      max_task_container_height: ""
    };

    this.calculateMaxContainerHeight = this.calculateMaxContainerHeight.bind(
      this
    );
  ...
  generateTaskCards(
    ...
    return (
      <div
        key={`container${i}`}
        className="day_task_container_wrapper"
        style={{
          gridColumn: { day_number },
          maxHeight: max_task_container_height
        }}
      >
        <div className="day_task_container">{task_cards}</div>
      </div>
    );
  }
  ...
  generateDayCells(
    ...
    day_date.setDate(
      aux_view_time.getDate() - ((aux_view_time.getDay() + 6) % 7)
    ); //week monday
    const day_names = [
      "monday",
      "tuesday",
  ...
  getHeaderDate(aux_view_time) {
    ...
    sunday_date.setDate(
      aux_view_time.getDate() + ((7 - aux_view_time.getDay()) % 7)
    );
    ...
    main_text +=
      " " + monday_date.toLocaleString("en-GB", { day: "numeric" }) + " -";
      ...
      <div className="content_card" id="content_card">
        <div className="content_header">
          {(() => {
            ...
            day_date.setDate(
              this.props.aux_view_time.getDate() -
                ((this.props.aux_view_time.getDay() + 6) % 7)
            );
            for (var i = 0; i < 7; i++) {
