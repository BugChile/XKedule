...
import { dateToWrittenDate } from "../../js_helpers/parsers.js";
import {
  rrule_day_dict,
  everyday_rrule,
  everyweekday_rrule,
  ...
  get_day_occurrence,
  getRepeatsSummary
} from "../../js_helpers/rrule_helpers.js";
...
export default class RepeatTool extends React.Component {
  constructor(props) {
    ...
    this.option_list = [
      ...
      yearly_on_month_day(
        props.event_date.getMonth() + 1,
        props.event_date.getDate()
      )
    ];
  ...
  receiveEndingResult(result) {
    ...
    if (ending_mode === "until specific date") {
      ...
      this.setState({
        ending_mode: "occurrences",
        ending_occurrences: result[1]
      });
    } else {
  ...
  setRRule(multipleStageOutput) {
    if (multipleStageOutput[0] === "custom...") {
      this.setState({
        rrule: this.generateCustomRRule(multipleStageOutput[1])
      });
    ...
    if (custom_tool_value.freq === 1) {
      ...
      if (custom_tool_value.month_mode === "on_day_number") {
        ...
        new_rrule = new RRule({
          ...
          interval: custom_tool_value.interval,
          byweekday: [
            rrule_day_dict[this.props.event_date.getDay()].nth(
              1 + get_day_occurrence(this.props.event_date)
            )
          ]
        });
      }
    } else if (custom_tool_value.freq === 2) {
      // weekly
      new_rrule = new RRule({
        freq: RRule.WEEKLY,
        interval: custom_tool_value.interval,
        byweekday: custom_tool_value.week_mode_selected.map(
          x => rrule_day_dict[x]
        )
      });
    } else {
  ...
  getEndingText(ending_mode) {
    if (ending_mode === "date") {
      return dateToWrittenDate(this.state.ending_date);
    } else if (ending_mode === "occurrences") {
      return ` after ${this.state.ending_occurrences} ${
        this.state.ending_occurrences > 1 ? "occurrences" : "occurrence"
      }`;
    } else {
      return "never";
    }
    ...
    return (
      <div className={this.props.className}>
        <div className="horizontal_flex label_input_combo" id="custom_row_2">
          <div className="input_label">Repeats</div>
          <OnOffInputContainer
        ...
        <div className="horizontal_flex label_input_combo" id="custom_row_2">
          <div className="input_label">Ending</div>
          <OnOffInputContainer
            on_component={MultipleStageInput}
            off_component={SimpleInputOffState}
              ...
              component_list: [
                {
                  ...
                  route_values: {
                    submit: "never",
                    go_to: {
                      "until specific date": 1, //go to calendar
                      "after number of occurrences": 2
                    } //go to occurrences tool
                  }
                },
                {
