...
export default class NumberSelector extends React.PureComponent {
  ...
  getValueDiv(keyboard_mode, value, keyboard_mode_value) {
    ...
    if (keyboard_mode) {
      ...
      value_div = <div {...props}>{this.parseValue(value)}</div>;
    }
    return value_div;
  }
  ...
  handleKeyboardInput(event) {
    ...
    if (
      parseInt(possible_new_value) >= this.min_value &&
      possible_new_value.length < 8
    ) {
  ...
  render() {
    return (
      <div className={"wheel_selector_container " + this.props.className}>
        <div className="wheel_selector_next_control" onClick={this.moveUp}>
          <svg
        ...
        <div className="wheel_selector_next_control" onClick={this.moveDown}>
          <svg
            className="control_arrow reversed"
            viewBox="0 0 114 60"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
