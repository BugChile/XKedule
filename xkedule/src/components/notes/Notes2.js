  ...
  function renderContent(keyGiven, index) {
    ...
    return Object.keys(children).map((key, index_) => {
      return (
        ...
        >
          {children[key].name} {keyGiven === key ? "hola" : null}
        </div>
      );
    });
  }
  ...
  return (
    ...
    >
      <div className="title_tab">
        Your mind is for{" "}
        <span className="text_bold_title  color_text_notes">having ideas</span>,
        not{" "}
        <span className="text_bold_title color_text_notes">holding them.</span>{" "}
      </div>
